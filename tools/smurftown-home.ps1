...
.DESCRIPTION
    ...
    checks data.yaml for real addresses, drive-hots reads app.yaml for the game path
    and data.yaml for a login, test-home creates the folder in the first place. If one of
    them resolved this differently from Battledeck/Directories.cs, it would look into one
    ...
    MUST STAY IN STEP WITH Battledeck/Directories.cs -> Resolve(): trimmed, environment
