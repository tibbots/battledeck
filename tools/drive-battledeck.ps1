<#
.SYNOPSIS
    Drives the RUNNING Battledeck window - clicks, keys, captures.
...
.DESCRIPTION
    ...
    BOUNDARY THIS SCRIPT DOES NOT CROSS: it does not start Battledeck. That stays with
...
.PARAMETER Lang
    ...
    script does not switch Battledeck's own UI language - set that before starting it.

.EXAMPLE
    .\tools\drive-battledeck.ps1 -Do 'front; click:139,140; wait:600; move:660,770; shot:filter-game'
    .\tools\drive-battledeck.ps1 -Lang de -Do 'front; move:20,20; shot:overview'
#>
[CmdletBinding()]
param(
...
if (-not $proc) { throw 'Battledeck is not running. Starting it is the user''s job - see CLAUDE.md.' }
