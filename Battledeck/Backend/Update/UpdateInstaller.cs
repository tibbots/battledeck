...
namespace Battledeck.Backend.Update
{
    ...
    public enum InstallRoute
    {
        ...
        ///     <c>C:\Program Files\ZrdJ\Battledeck</c> the old MSI produced - it needs
    ...
    ///         application is <c>Battledeck.exe</c>, there is no set of DLLs beside it that
    ...
    public static class UpdateInstaller
    {
        ...
        /// </summary>
        private const string ExeName = "Battledeck.exe";
        ...
        public static InstallRoute Route()
        {
            ...
            // Battledeck.dll lying beside it is therefore a build out of bin\Debug or
            ...
            if (File.Exists(Path.Combine(folder, "Battledeck.dll"))) return InstallRoute.DevBuild;
            ...
            GithubRelease release, IProgress<UpdateProgress>? progress, CancellationToken cancel)
        {
            ...
            var work = Path.Combine(Path.GetTempPath(), "battledeck-update");
        ...
        ///     <para>
        ///         <b>Only <c>Battledeck.exe</c>.</b> The package also carries the
        ...
        ///         moves there is a moment in which no <c>Battledeck.exe</c> exists; if the
        ...
        {
            var probe = Path.Combine(folder, $"battledeck-write-probe-{Guid.NewGuid():N}");
