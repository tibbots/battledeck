...
public class TextsTests
{
    ...
    private static readonly Regex FinePattern = new(
        @"^([#_x\[\]\s;&]|&#x[0-9A-Fa-f]+;|\d)*$" +
        @"|^(Battledeck|BATTLEDECK|HEROES OF THE STORM|Heroes of the Storm|Battle\.net" +
        @"|EU|AM|AS|OK|Height)$");
    ...
    {
        var resourceName = $"Battledeck.Backend.Texts.{tag}.yaml";
    ...
    ///     <c>Battledeck.sln</c> turns up, or gives up at the filesystem root. Checks 3 and 4
    ...
    private static string? FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "Battledeck.sln")))
    ...
    {
        var sourceRoot = Path.Combine(repoRoot, "Battledeck");
    ...
    public void Every_key_used_in_the_code_is_in_english()
    {
        ...
        if (repoRoot is null)
        {
            ...
            throw Xunit.Sdk.SkipException.ForSkip(
                "Could not locate the source tree (a directory containing Battledeck.sln) by " +
    ...
    public void No_visible_text_in_xaml_bypasses_the_translation()
    {
        ...
        var scanned = Directory
            .EnumerateFiles(Path.Combine(repoRoot, "Battledeck"), "*.xaml", SearchOption.AllDirectories)
