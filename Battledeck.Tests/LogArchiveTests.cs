...
namespace Battledeck.Tests
{
    ...
    public class LogArchiveTests
    {
        ...
        public void A_log_that_is_no_longer_written_to_becomes_an_archive()
        {
            ...
            Log(folder, "battledeck_001.log", "the one that rolled");

            LogArchive.Sweep(current);

            Assert.False(File.Exists(Path.Combine(folder, "battledeck_001.log")));

            using var zip = ZipFile.OpenRead(Path.Combine(folder, "battledeck_001.log.zip"));
            using var reader = new StreamReader(zip.GetEntry("battledeck_001.log")!.Open());
        ...
        public void An_archive_is_never_swept_into_another_archive()
        {
            ...
            var archive = Path.Combine(folder, "battledeck_001.log.zip");
            LogArchive.CompressInto(Log(folder, "battledeck_001.log", "rolled"), archive);
            File.Delete(Path.Combine(folder, "battledeck_001.log"));
            ...
            Assert.Equal(["battledeck_001.log"], EntryNames(archive));
        }
        ...
        public void Only_as_many_archives_survive_as_fit_beside_the_current_file()
        {
            ...
            for (var i = 0; i < written; i++)
                Stamp(Path.Combine(folder, $"battledeck_{i:000}.log.zip"), TimeSpan.FromMinutes(i));
            ...
            Assert.Contains($"battledeck_{written - 1:000}.log.zip", left);
            Assert.DoesNotContain("battledeck_000.log.zip", left);
        }
        ...
        public void A_half_written_archive_does_not_block_the_next_attempt()
        {
            var folder = FreshFolder();
            var current = Log(folder, "smurftown.log", "current");
            Log(folder, "battledeck_001.log", "rolled");
            ...
            File.WriteAllText(Path.Combine(folder, "battledeck_001.log.zip.partial"), "half a zip");

            LogArchive.Sweep(current);

            Assert.False(File.Exists(Path.Combine(folder, "battledeck_001.log.zip.partial")));
            Assert.Equal(["battledeck_001.log"], EntryNames(Path.Combine(folder, "battledeck_001.log.zip")));
        }
        ...
        {
            var folder = FreshFolder();
            Log(folder, "battledeck_001.log", "written before the process died");
            ...
            Assert.True(File.Exists(Path.Combine(folder, "battledeck_001.log.zip")));
        }
