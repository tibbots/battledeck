---
name: drive-battledeck
version: 1
description: Operate the RUNNING Battledeck window from outside — clicks, keys, mouse wheel, captures — via tools/drive-battledeck.ps1. Use for README screenshots and for looking at a UI change in the real app. Triggers on drive Battledeck, click in the app, screenshot the app, operate the UI, "App fernsteuern", drive-battledeck.
---

# Driving the running Battledeck window

`tools/drive-battledeck.ps1` clicks, types, scrolls and captures in an **already running**
Battledeck. It exists because the UI has to be in a specific state for each README shot, and
...
**The script never starts Battledeck.** If none is running it aborts instead of starting one, and
...
**first** Battledeck process that owns a window — with two of them up, which window gets clicked is a
